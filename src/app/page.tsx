...
export default function Home() {
  ...
  useLayoutEffect(() => {
    ...
    const nextValue = savedTheme === 'light' ? false : savedTheme === 'dark' ? true : systemPrefersDark
      ...
      <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="text-center space-y-4">
          <div className="relative mx-auto h-12 w-12">
            ...
            <div className="relative flex items-center justify-center h-12 w-12 rounded-full bg-gradient-to-tr from-violet-600 to-indigo-600">
              <span className="text-white font-extrabold text-sm">ใช้</span>
            </div>
          </div>
          <p className="text-xs font-medium text-slate-400">กำลังเตรียมพร้อม...</p>
        </div>
      </div>
    )
  }
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'wallet':   return <WalletTab userId={user.id} />
      case 'tasks':    return <TasksTab userId={user.id} />
      case 'schedule': return <ScheduleTab userId={user.id} />
      case 'diary':    return <DiaryTab userId={user.id} />
    }
  }

  return (
    <div className={`min-h-screen transition-colors duration-300 moving-gradient-bg ${isDarkMode ? 'text-slate-100 dark' : 'text-slate-900'}`}>
      ...
      <aside className="sidebar-nav hidden lg:flex flex-col border-r border-slate-200/60 dark:border-slate-800/60 bg-white/80 dark:bg-slate-950/90 backdrop-blur-xl transition-colors">
        ...
        <div className="flex items-center gap-2.5 px-6 h-16 border-b border-slate-200/60 dark:border-slate-800/60">
          <span className="text-lg font-bold tracking-tight bg-linear-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-300 bg-clip-text text-transparent">
            ใช้เอง
          </span>
        </div>
        ...
        <div className="border-t border-slate-200/60 dark:border-slate-800/60 p-4 space-y-3">
          ...
          <div className="flex items-center gap-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 p-3">
            {userAvatar ? (
              <img src={userAvatar} alt={userName} className="h-9 w-9 rounded-xl object-cover shrink-0" />
            ) : (
              <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center text-xs font-bold text-white uppercase shrink-0">
                {userEmail[0]}
              </div>
            )}
            ...
            <button
              ...
              className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-500/5 transition-all"
              title="ออกจากระบบ"
            >
      ...
      <div className="main-with-sidebar">
        ...
        <header className="sticky top-0 z-40 border-b border-slate-200/60 dark:border-slate-900/80 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl transition-colors lg:hidden">
          <div className="content-container flex h-14 items-center justify-between">
            ...
            <div className="flex items-center gap-2.5">
              <span className="text-[17px] font-bold tracking-tight bg-linear-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-300 bg-clip-text text-transparent">
                ใช้เอง
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                >
                  {userAvatar ? (
                    <img src={userAvatar} alt={userName} className="h-6 w-6 rounded-lg object-cover" />
                  ) : (
                    <div className="h-6 w-6 rounded-lg bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center text-[10px] font-bold text-white uppercase">
                      {userEmail[0]}
                    </div>
                  )}
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-2 z-50 w-60 rounded-2xl border border-slate-200/60 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xl animate-scale-up">
                      <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-3 mb-3">
                        {userAvatar ? (
                          <img src={userAvatar} alt={userName} className="h-10 w-10 rounded-xl object-cover shrink-0" />
                        ) : (
                          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center text-sm font-bold text-white uppercase shrink-0">
                            {userEmail[0]}
                          </div>
                        )}
                      ...
                      <button
                        onClick={handleSignOut}
                        className="flex w-full items-center justify-center gap-2 rounded-xl bg-red-500/10 py-2.5 text-xs font-semibold text-red-500 dark:text-red-400 border border-red-500/10 hover:bg-red-500/20 transition-colors"
                      >
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                        </svg>
        ...
        {/* ── Main Content ── */}
        <main className="content-container pt-5">
          {renderTabContent()}
        </main>
      </div>
          ...
          {TABS.map(tab => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 h-0.5 w-8 rounded-full bg-indigo-500" />
                )}
                <div className={`transition-transform duration-200 ${isActive ? 'scale-110' : ''}`}>
                  {tab.icon}
                </div>
